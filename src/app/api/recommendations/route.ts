...
export async function GET(request: NextRequest) {
  try {
    ...
    // Try fetching personalized recommendations from ML service
    ...
    try {
      ...
      const contextText =
        recentMood?.notes?.trim() ||
        recentJournal?.content?.trim() ||
        `I am feeling ${mood.toLowerCase()} today.`;
      ...
      // Ignore ML fetch error and fall back to templates
    }
    ...
    if (mlRecs && mlRecs.length > 0) {
      itemsToCreate = mlRecs.map((r) => ({
        type: typeMapping[r.type] || "MEDITATION",
        title: r.title,
        description: r.description,
        reason: `Generated by MindEase ML for your current ${mood.toLowerCase()} state`,
      }));
    } else {
      itemsToCreate = all.slice(0, 5).map((r) => ({
        ...r,
